...
int main(int argv, char* argc)
{
	...
	if (opcja_obrazka == 1)
	{
		...
		Obrazek = cv::imread(nazwa_pilku_zaszumionego, cv::IMREAD_GRAYSCALE);
	...
	printf("Uplynelo %.2f sek.", difftime(czasStop, czasStart));
	std::cout << "Wykonanie trwalo: " << difftime(czasStop, czasStart) << " sekund"<<std::endl;
	Obrazek.convertTo(Obrazek, CV_8U);
